...
def get_dataloader(path, max_len, batch_size, shuffle, bert_path, data_type, language):
    ...
    if data_type == 'rationale':
        ...
        for item in data_list:
            ...
            # 检查字段是否有为空的值
            if None in tmp_data.values() or '' or 'N/A' in tmp_data.values():
                continue  # 如果有空值则跳过该条数据

            # 将非空数据添加到 DataFrame
            df_data = df_data.append(tmp_data, ignore_index=True)

        # 删除 DataFrame 中任何包含 NaN 或 None 的行
        df_data = df_data.dropna()
        print(df_data['FTR_2_pred'].isnull().sum())  # 检查缺失值的数量h
        ...
        FTR_2_pred = torch.tensor(df_data['FTR_2_pred'].apply(lambda c: label_dict_ftr_pred[c] if c != 'N/A' else 0).astype(int).to_numpy())
        FTR_3_pred = torch.tensor(df_data['FTR_3_pred'].apply(lambda c: label_dict_ftr_pred[c] if c != 'N/A' else 0).astype(int).to_numpy())
